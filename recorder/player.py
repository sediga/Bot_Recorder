...
async def handle_step(step: dict, page: Page, all_steps: list):
    ...
    if step_type == "navigate":
        ...
        loop_name = step.get("name")
        source_id = step.get("source")
        actions = step.get("actionsPerRow", [])
        extract = getattr(page.context, "_botflows_extractions", {}).get(source_id)
        if not extract:
            logger.warning(f"[loop] Extract step '{source_id}' not found.")
            return
        ...
        logger.info(f"[loop: {loop_name}] Found {len(rows)} rows")
        for idx, row in enumerate(rows):
            logger.info(f"[loop: {loop_name}] Row {idx + 1}")
            for action in actions:
                sel = action.get("selector") or action.get("improvedSelector")
        ...
        for i in range(count):
            ...
            for sub_step in step.get("steps", []):
                await handle_step(sub_step, page, all_steps)
        ...
        if not extract:
            ...
            return
        grid_selector = extract.get("gridSelector")
        row_selector = extract.get("rowSelector", "tr")
        ...
        try:
            ...
            if filters:
                ...
                for row in rows:
                    ...
                    if all(filt["value"].lower() in row_text.lower() for filt in filters):
                        filtered_rows.append(row)
                rows = filtered_rows
                logger.info(f"{len(rows)} rows remain after filtering")

            temp_table = []
            child_steps = [s for s in all_steps if s.get("parentId") == step.get("id")]
            for idx, row in enumerate(rows):
                logger.info(f"Processing row {idx + 1}/{len(rows)}")
                row_data = {}
                for action in child_steps:
                    action_type = action.get("action")
                    relative_selector = action.get("selector", "")
                    if relative_selector.startswith("div[") or relative_selector.startswith(".") or " " in relative_selector:
...
async def replay_flow(json_str: str):
    ...
    async with async_playwright() as p:
        ...
        for step in flow:
            if step.get("parentId"):  # skip child steps here; they'll be handled by parent
                continue
            await handle_step(step, page, flow)
